...
const engageButton= $("#engage-button");
...
export function changeSessionValues(event){
	const target = $(event.target);
	let timerNumber;
	//decreases the break or work time by one
	if (target.is(subtractTime)){
		target.next().html((index, oldhtml) => {
	...
	if (target.is(addTime)){
		target.prev().html((index, oldhtml) => {
			timerNumber = parseInt(oldhtml) + 1 ;
			return timerNumber ;
		});
	}
...
export function pauseCountDown(event) {
	const target = $(event.target);
...
export function resetTimer(){
	...
	$("#timer-clock").html((index, oldhtml) => $("#work-time").html());
...
export function startCountDown(event){
	...
	const workDuration = $("#work-time");
	const breakDuration = $("#break-time");
...
export function timerChange(workTime,breakTime){
	// variables which mark the beginning states of the Work and Break time
	const initialWorkTime = `${workTime.html()}:00`;
	const initialBreakTime = `${breakTime.html()}:00`;
	...
	const workLoadInterval = 410 / (parseInt(initialWorkTime) * 60);
  const breakLoadInterval = 410 / (parseInt(initialBreakTime) * 60);
    ...
    loopingTimer  = window.setInterval(() => {
    	//min and secs assign the min and seconds strings to be used later in the function
    	const min = timerClock.html().split(":")[0];
    	const secs = timerClock.html().split(":")[1];
		...
		if(secs === "00"){
			timerClock.html((index, oldhtml) => `${parseInt(min) -1}:${60 - 1}`);
		}
		...
		if(secs !== "00"){
			timerClock.html((index, oldhtml) => {
				let seconds = parseInt(secs) - 1;
				if (seconds < 10){
					seconds = `0${seconds}`;
				}
				return `${min}:${seconds}` ;
